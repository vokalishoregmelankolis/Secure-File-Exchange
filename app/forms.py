...
class AccessRequestForm(FlaskForm):
    """Form for submitting access request"""
    message = TextAreaField('Request Message', validators=[Optional(), Length(max=500)])
    submit = SubmitField('Submit Request')


class DecryptKeyForm(FlaskForm):
    """Form for decrypting private key"""
    password = PasswordField('Password', validators=[DataRequired()])
    submit = SubmitField('Decrypt Key')